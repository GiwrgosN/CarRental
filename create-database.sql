...
foreign key (IRS_Number) references EMPLOYEE(IRS_Number) ON DELETE CASCADE ON UPDATE CASCADE,
foreign key (Store_ID) references STORE (Store_ID) ON DELETE CASCADE ON UPDATE CASCADE
);
...
FOREIGN KEY (Start_Location) references STORE (Store_ID) ON DELETE SET NULL ON UPDATE CASCADE,
FOREIGN KEY (Finish_Location) references STORE (Store_ID) ON DELETE SET NULL ON UPDATE CASCADE
);
...
IRS_Number BIGINT unsigned,
PRIMARY KEY (Start_Date,License_Plate),
FOREIGN KEY (Start_Date) REFERENCES RESERVES (Start_Date) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (License_Plate) REFERENCES RESERVES (License_Plate) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (Customer_ID) REFERENCES CUSTOMER (Customer_ID) ON DELETE RESTRICT ON UPDATE CASCADE,
FOREIGN KEY (IRS_Number) REFERENCES EMPLOYEE (IRS_Number) ON DELETE SET NULL ON UPDATE CASCADE,
FOREIGN KEY (Start_Location) REFERENCES STORE (Store_ID) ON DELETE SET NULL ON UPDATE CASCADE,
FOREIGN KEY (Finish_Location) REFERENCES STORE (Store_ID) ON DELETE SET NULL ON UPDATE CASCADE
);
...
PRIMARY KEY (Start_Date,License_Plate),
FOREIGN KEY (Start_Date) REFERENCES RENTS (Start_Date) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (License_Plate) REFERENCES RENTS (License_Plate) ON DELETE CASCADE ON UPDATE CASCADE
);
...
BEGIN
    ...
    IF (NEW.Finish_Date <> OLD.Finish_Date) THEN
        IF EXISTS (SELECT License_Plate
                   FROM RESERVES
                   WHERE NEW.License_Plate = License_Plate AND Start_Date <> OLD.Start_Date AND ((Start_Date <= NEW.Start_Date AND NEW.Start_Date <= Finish_Date) OR (Start_Date <= NEW.Finish_Date AND NEW.Finish_Date <= Finish_Date) OR (NEW.Start_Date <= Start_Date AND NEW.Finish_Date >= Finish_Date))) THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Vehicle is not available for those Dates';
        END IF;
    END IF;
END; $$
